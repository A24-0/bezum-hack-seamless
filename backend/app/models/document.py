...
class DocumentApprovalDecision(str, enum.Enum):
    approved = "approved"
    rejected = "rejected"
...
class DocumentApproval(Base):
    ...
    version_num: Mapped[int] = mapped_column(Integer, nullable=False, index=True)
    user_id: Mapped[int] = mapped_column(Integer, ForeignKey("users.id", ondelete="CASCADE"), nullable=False)
    decision: Mapped[DocumentApprovalDecision] = mapped_column(Enum(DocumentApprovalDecision), nullable=False)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now())
